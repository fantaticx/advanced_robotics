...
void GazeboRosP3D::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (!_sdf->HasElement("gaussianNoise"))
  {
    ...
    this->gaussian_noise_linear_ = 0;
    this->gaussian_noise_angular_ = 0;
  }
  else
  {
    this->gaussian_noise_linear_ = _sdf->GetElement("gaussianNoise")->Get<double>();
    this->gaussian_noise_angular_ = _sdf->GetElement("gaussianNoise")->Get<double>();
  }

  if (_sdf->HasElement("gaussianNoiseLinear"))
  {
    ROS_DEBUG_NAMED("p3d", "p3d plugin found <gaussianNoiseLinear>");
    this->gaussian_noise_linear_ = _sdf->GetElement("gaussianNoiseLinear")->Get<double>();
  }
  if (_sdf->HasElement("gaussianNoiseAngular"))
  {
    ROS_DEBUG_NAMED("p3d", "p3d plugin found <gaussianNoiseAngular>");
    this->gaussian_noise_angular_ = _sdf->GetElement("gaussianNoiseAngular")->Get<double>();
  }
  ...
  // initialize odometry with zero position
  this->pose_msg_.pose.pose.position.x    = previous_pose_.Pos().X();
  this->pose_msg_.pose.pose.position.y    = previous_pose_.Pos().Y();
  this->pose_msg_.pose.pose.position.z    = previous_pose_.Pos().Z();

  this->pose_msg_.pose.pose.orientation.x = previous_pose_.Rot().X();
  this->pose_msg_.pose.pose.orientation.y = previous_pose_.Rot().Y();
  this->pose_msg_.pose.pose.orientation.z = previous_pose_.Rot().Z();
  this->pose_msg_.pose.pose.orientation.w = previous_pose_.Rot().W();
}
  ...
  if (this->pub_.getNumSubscribers() > 0)
  {
    ...
    if (tmp_dt != 0)
    {
      ...
      if (this->topic_name_ != "")
      {
        ...
        if (this->gaussian_noise_linear_ > 0.000001 || this->gaussian_noise_angular_ > 0.000001)
        {
          ignition::math::Pose3d diff_pose;
          diff_pose.Pos() = pose.Pos() - previous_pose_.Pos();
          diff_pose.Rot() = pose.Rot() * previous_pose_.Rot().Inverse();
          diff_pose.Rot().Normalize();
          ignition::math::Vector3d diff_euler = diff_pose.Rot().Euler();
          double min_diff = 0.01;

          double gauss_linear = (std::abs(diff_pose.Pos().X()) > min_diff || std::abs(diff_pose.Pos().Y()) > min_diff) ? this->GaussianKernel(0, this->gaussian_noise_linear_) : 0.0;
          double gauss_angular_y = std::abs(diff_euler.Z()) > min_diff ? this->GaussianKernel(0, this->gaussian_noise_angular_) : 0.0;
          // add gaussian noise to yaw orientation
          diff_euler.Set(0.0, 0.0, diff_euler.Z() + gauss_angular_y);
          diff_pose.Rot().Set(this->pose_msg_.pose.pose.orientation.w, 
                              this->pose_msg_.pose.pose.orientation.x, 
                              this->pose_msg_.pose.pose.orientation.y, 
                              this->pose_msg_.pose.pose.orientation.z);
          diff_pose.Rot() *= diff_pose.Rot().EulerToQuaternion(diff_euler);
          diff_pose.Rot().Normalize();

          // add gaussian to position
          double vec_length = std::sqrt(diff_pose.Pos().X() * diff_pose.Pos().X() + diff_pose.Pos().Y() * diff_pose.Pos().Y()) + gauss_linear;

          if (pose.Rot().Euler().Z() >= 0.0 && pose.Rot().Euler().Z() < M_PI / 2 && diff_pose.Pos().X() < 0.0)
          {
            vec_length *= -1;
          }
          else if (pose.Rot().Euler().Z() >= M_PI / 2 && pose.Rot().Euler().Z() <= M_PI && diff_pose.Pos().Y() < 0.0)
          {
            vec_length *= -1;
          }
          else if (pose.Rot().Euler().Z() < 0.0 && pose.Rot().Euler().Z() >= -(M_PI / 2) && diff_pose.Pos().Y() > 0.0)
          {
            vec_length *= -1;
          }
          else if (pose.Rot().Euler().Z() < -(M_PI / 2) && pose.Rot().Euler().Z() >= -M_PI && diff_pose.Pos().X() > 0.0)
          {
            vec_length *= -1;
          }

          double x_diff = vec_length * std::cos(diff_pose.Rot().Euler().Z());
          double y_diff = vec_length * std::sin(diff_pose.Rot().Euler().Z());
          diff_pose.Pos().Set(x_diff, y_diff, diff_pose.Pos().Z());

        // Fill out messages
          this->pose_msg_.pose.pose.position.x    += diff_pose.Pos().X();
          this->pose_msg_.pose.pose.position.y    += diff_pose.Pos().Y();
          this->pose_msg_.pose.pose.position.z    = pose.Pos().Z();

          this->pose_msg_.pose.pose.orientation.x = diff_pose.Rot().X();
          this->pose_msg_.pose.pose.orientation.y = diff_pose.Rot().Y();
          this->pose_msg_.pose.pose.orientation.z = diff_pose.Rot().Z();
          this->pose_msg_.pose.pose.orientation.w = diff_pose.Rot().W();

          this->pose_msg_.twist.twist.linear.x  = diff_pose.Pos().X();
          this->pose_msg_.twist.twist.linear.y  = diff_pose.Pos().Y();
          this->pose_msg_.twist.twist.linear.z  = vpos.Z();
          // pass euler angular rates
          this->pose_msg_.twist.twist.angular.x = diff_euler.X();
          this->pose_msg_.twist.twist.angular.y = diff_euler.Y();
          this->pose_msg_.twist.twist.angular.z = diff_euler.Z();
        }
        else
        {
        ...
        double gn2 = this->gaussian_noise_linear_*this->gaussian_noise_linear_;
